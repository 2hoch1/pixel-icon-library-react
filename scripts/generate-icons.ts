...
function buildIconFileContent(entry: IconEntry) {
  return [
    ...
    `  const handleRef = useCallback((svgElement: SVGSVGElement | null) => {`,
    `    if (typeof ref === 'function') {`,
    `      ref(svgElement);`,
    `    } else if (ref) {`,
    `      ref.current = svgElement;`,
    `    }`,
    `    // Set fill to currentColor on all child elements`,
    `    if (svgElement) {`,
    `      svgElement.querySelectorAll('polygon, path, circle, rect, ellipse, line, polyline').forEach((el) => {`,
